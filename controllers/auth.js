...
const register = async (req, res) => {
    try {
        ...
        if (user) {
            return res.status(500).json({ message: "Bu mail hesabı zaten kullanılmakta" })
        }
        if (password.length < 6) {
            return res.status(500).json({ message: "Paralonız 6 karakterden küçük olmaz" })
        }
...
const login = async (req, res) => {
    try {
        ...
        if (!comparePassword) {
            ...
            return res.status(500).json({ message: "Şifreler Uyuşmuyor." })
        }
